...
public class SoulParticle extends SimpleAnimatedParticle {
	private SoulParticle(ClientLevel world, double x, double y, double z, double motionX, double motionY, double motionZ, SpriteSet sprites) {
		super(world, x, y, z, sprites, 0.0f);
		xd = (float) motionX;
		yd = (float) motionY;
		zd = (float) motionZ;
		quadSize *= 4.0f + random.nextFloat() * 2.0f;
		lifetime = 5 + random.nextInt(2);
		gravity = 0.0f;
	...
	public static class Factory implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			SoulParticle p = new SoulParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, spriteSet);
			return (Particle) p;
		}
	}
}
