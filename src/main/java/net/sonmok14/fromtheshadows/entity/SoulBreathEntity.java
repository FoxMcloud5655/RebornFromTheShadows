...
public class SoulBreathEntity extends AbstractHurtingProjectile {
	protected int timeInAir;
	protected boolean inAir;
	private int ticksInAir;
	private LivingEntity shooter;

	public SoulBreathEntity(EntityType<? extends SoulBreathEntity> p_i50160_1_, Level p_i50160_2_) {
		super(p_i50160_1_, p_i50160_2_);
	}

	public SoulBreathEntity(Level worldIn, LivingEntity shooter, double accelX, double accelY, double accelZ) {
		super(EntityRegistry.SOUL_BREATH.get(), shooter, accelX, accelY, accelZ, worldIn);
		...
		if (level.isClientSide || ((entity == null || !entity.isRemoved()) && level.hasChunkAt(blockPosition()))) {
			...
			if (level.isClientSide) {
				for (int i = 0; i < 15; ++i) {
					...
					level.addParticle((ParticleOptions) ParticleRegistry.SOUL.get(), getX() - vec4.x, getY() - vec4.y + 0.15, getZ() - vec4.z, 0.1, 0.1, 0.1);
				}
			}
	...
	private void roar() {
		if (isAlive()) {
			for (LivingEntity livingentity : level.getEntitiesOfClass(LivingEntity.class, getBoundingBox().inflate(3.0))) {
				if (!(livingentity instanceof NehemothEntity)) {
					livingentity.hurt(DamageSource.mobAttack(shooter), 4.0f);
				}
			}
	...
	private void smash(int distance) {
		...
		double spread = 3.141592653589793;
